...
describe ZendeskAppsSupport::Validations::Svg do
  def mock_package(svg_file)
    double('Package', svg_files: [svg_file], warnings: [])
  end
...
stroke-linecap="round" d="M1.5 1.5h11m-10 8h9m-7-2h5m-5 4h5m-8-6h11m-8-2h5"></path></symbol></svg>)
      ...
      it 'are left unchanged with no warnings or errors' do
        svg_file = double('SVGFile', read: markup)
        package = mock_package(svg_file)
...
5v9.7L13 23l-9-5.2zm9 5L4 8m9 5l9-5m-9 5v10"></path></svg>\n)
      ...
      it 'are sanitised, with a warning that the offending svgs were modified' do
        svg_file = double('SVGFile', read: markup, relative_path: 'assets/icon.svg', absolute_path: '/tmp/icon.svg')
        package = mock_package(svg_file)

        errors = ZendeskAppsSupport::Validations::Svg.call(package)

        clean_markup = %(<svg viewBox="0 0 26 26" id="zd-svg-icon-26-app" width="100%" height="100%"><path fill="none" \
stroke="currentColor" stroke-linejoin="round" stroke-width="2" d="M4 8l9-5 9 5v9.7L13 23l-9-5.2zm9 5L4 8m9 \
5l9-5m-9 5v10"/></svg>)
        expect(IO).to have_received(:write).with('/tmp/icon.svg', clean_markup)
        expect(package.warnings[0]).to eq(
          'The markup in assets/icon.svg has been edited for use in Zendesk and may not display as intended.'
        )
        expect(errors).to be_empty
      end
    end
    ...
    it 'removes <script> tags' do
      markup = %(<svg viewBox="0 0 26 26" id="zd-svg-icon-26-app" width="100%" height="100%"><path fill="none" \
stroke="currentColor" stroke-linejoin="round" stroke-width="2" d="M4 8l9-5 9 5v9.7L13 23l-9-5.2zm9 5L4 8m9 \
...
d="M4 8l9-5 9 5v9.7L13 23l-9-5.2zm9 5L4 8m9 5l9-5m-9 5v10"></path></svg onResize>\n)
      ...
      it 'have the contents of their svg tags emptied, with a warning that the offending svgs were modified' do
        ...
        empty_svg = %(<svg/>)
        expect(IO).to have_received(:write).with('/tmp/icon.svg', empty_svg)
        expect(package.warnings[0]).to eq(
          'The markup in assets/icon.svg has been edited for use in Zendesk and may not display as intended.'
        )
        expect(errors).to be_empty
  ...
  context 'svgs that contain embedded bitmap(s)' do
    it 'generate a warns the developer, and overwrites the svg with a placeholder image' do
      markup = %(<svg viewBox="0 0 26 26" id="1px-white-square" width="100%" height="100%"><image width="1" height="1" x="0" \
y="0" xlink:href="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="/></svg>)
      ...
      placeholder_markup = %(<?xml version="1.0" encoding="utf-8"?><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" \
...
<path id="Fill-3" d="M1.5,6.1C1.3,5.9,1,6,1,6.4l0,7c0,0.3,0.2,0.7,0.5,0.9l6,3.6C7.8,18.1,8,18,8,17.6l0-7.1c0-0.3-0.2-0.7-0.5-0.9 L1.5,6.1z"/>\
<path id="Fill-5" d="M10.5,17.9c-0.3,0.2-0.5,0-0.5-0.3l0-7c0-0.3,0.2-0.7,0.5-0.9l6-3.6C16.8,5.9,17,6,17,6.4l0,7.1 c0,0.3-0.2,0.7-0.5,0.9L10.5,17.9z"/>\
<path id="Fill-1" d="M2.2,3.7c-0.3,0.2-0.3,0.4,0,0.6l6.2,3.6C8.7,8,9.2,8,9.4,7.9l6.3-3.6c0.3-0.2,0.3-0.4,0-0.6L9.5,0.1 C9.2,0,8.8,0,8.5,0.1L2.2,3.7z"/></svg>)
      expect(IO).to have_received(:write).with('/tmp/icon.svg', placeholder_markup)
      expect(package.warnings[0]).to eq(
        'assets/icon.svg contains an embedded bitmap and cannot be used as an app icon. It has been replaced with a default placeholder icon.'
      )
      expect(errors).to be_empty
    end
  end
...
5l9-5m-9 5v10" onclick="alert(1)"></path></svg>\n)
      ...
      expect(errors[0].data).to eq(svg: 'assets/icon.svg')
    end
  end
end
